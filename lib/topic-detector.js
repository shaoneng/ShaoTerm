...
function parseAiModelFromCommand(commandValue) {
  ...
  const gptTokenMatch = command.match(/\b(gpt-[a-z0-9.\-_]+)\b/i);
  if (gptTokenMatch && gptTokenMatch[1]) {
    return gptTokenMatch[1].trim();
  }
...
function resolveAnalysisBackend() {
  const providerHint = inferProviderFromAiCommand(aiCommand);
  const normalizedBaseUrl = normalizeBaseUrlForRequests(baseUrl);
...
function resolveOpenAIHeartbeatModel() {
  return parseAiModelFromCommand(aiCommand) || DEFAULT_OPENAI_HEARTBEAT_MODEL;
}
...
async function createOpenAICompatibleCompletion(messages, maxTokens = 180) {
  const normalizedBaseUrl = normalizeBaseUrlForRequests(baseUrl);
  if (!normalizedBaseUrl) {
  ...
  try {
    ...
    const response = await fetch(requestUrl, {
      ...
      body: JSON.stringify({
        model: resolveOpenAIHeartbeatModel(),
